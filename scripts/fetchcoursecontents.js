...
// The 'files' array will be dynamically populated from GitHub.
...
async function generateCourseContentLayout(courseId, contentContainerId, introTextId, githubRepoPath) {
    ...
    try {
        ...
        // Store the fetched files globally for potential "Next" button logic on lesson pages
        ...
        if (contentContainer) {
            ...
            lessonFiles.forEach(item => {
                ...
                button.href = `./contents/${item.name}`;
 ...
 */
async function setupLessonNavigation(courseId, currentLessonFileName, nextButtonId, githubRepoPath) {
    ...
    if (!lessonFiles || window.currentCourseId !== courseId) {
        try {
            ...
            nextButton.style.display = 'none'; // Hide button if list cannot be fetched
            return;
        }
    }
    ...
    if (currentIndex !== -1 && currentIndex < lessonFiles.length - 1) {
        ...
        // Relative path from the current lesson file to the next lesson file
        nextButton.href = `./${nextLesson.name}`;
        nextButton.style.display = 'inline-block'; // Show the button
    } else {
        nextButton.style.display = 'none'; // Hide if it's the last lesson
    }
}
